...
$titlehead8 = 'Reserve Officers Training Corps';
...
$header = '#, Name, Course, Attendance, Prelim, Midterm, Finals, PX, Aptitude, ATD, SP, APT, TWA, Rating, Remarks';
...
foreach($datas['result'] as $k => $v) {
	foreach($datax['result'] as $kk => $vv){
 	...
 	$body .= $count.','
			...
			.'=i'.$d16. ','
			.'=(j'.$d16.'*'.$a1.'/10) + (k'.$d16.'*'.$a7.') + (l'.$d16.'*'.$a5.'/10)'. ','
			.'=if(m'.$d16.'>=95'. $j .'"1.0"'. $j .'if(m'.$d16.'>=91'. $j .' "1.25"'.$j.
			  'if(m'.$d16.'>=88'.$j.'"1.50"'.$j.'if(m'.$d16.'>=86'.$j.'"1.75"'.$j.'if(m'.$d16.'>=84'.$j.'"2.0"'.$j.
			  'if(m'.$d16.'>=82'.$j.'"2.25"'.$j.'if(m'.$d16.'>=79'.$j.'"2.50"'.$j.
			  'if(m'.$d16.'>=77'.$j.'"2.75"'.$j.'if(m'.$d16.'>=75'.$j.'"3.0"'.$j.
			  'if(m'.$d16.'>=70'.$j.'"6.0"'.$j.'if(m'.$d16.'<70'.$j.'"5.0"))))))))))),'
			.'=if(m'.$d16.'<=69'. $j .'"Failed"'. $j. 
			   'if(m'.$d16.'<=74'. $j .' "Incomplete" '. $j .' "Passed" '.'))'.','."\n";
		}
            $count++;
            $d16++;
            $sales = $salary++; 
