...
$titlehead14 = '';
$header = '#, Name, Course, Attendance, Prelim, Midterm, Finals, Aptitude, Project, ATD, SP, APT, PRJ, TWA, Rating, Remarks';
$subheader = ', , , , , , , , ,';
$count = 1;
$d16 = 16;
$d17 = '=d'.$d16;
...
foreach($datas['result'] as $k => $v) {
	foreach($datax['result'] as $kk => $vv){
 	...
 	$body .= $count.','
			...
			.'=h'.$d16. ','
			.'=i'.$d16. ','
			.'=(j'.$d16.'*'.$a1.'/10) + (k'.$d16.'*'.$a7.') + (l'.$d16.'*'.$a5.'/10)+(m'.$d16.'*'.$a6.'/10)'. ','
			.$v['ratings'].','
	
			
				.'=if(n'.$d16.'<=69'. $j .'"Failed"'. $j. 'if(n'.$d16.'<=74'. $j .' "Incomplete" '. $j .' "Passed" '.'))'.','."\n";
